...
describe("<CardList />", () => {
  ...
  it("CHECK: If dose not exist 'getImageAlt', 'alt' will be 'name'", () => {
    ...
    render(
      <CardList
        items={items}
        getImageSrc={(it) => `/sprites/${it.id}.png`}
      />
    );
  ...
  it("CHECK: If it has 'className'", () => {
    const items: ListItem[] = [{ id: "1", name: "bulbasaur", href: "/pokemon/1" }];

    const { container } = render(<CardList items={items} className="testClass" />);
    const ul = container.querySelector("ul")!;
    expect(ul.className).toMatch(/testClass/);
  });
});
